...
export default function SettingsPage() {
    ...
    const connectOneDrive = async () => {
        if (!user) return;
        const state = await fetch("/api/auth/universalState", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
        ...
        router.push(`/api/auth/onedrive/auth?state=${data.state}`);
    };
    ...
    const disconnectOneDrive = async () => {
        ...
        const data = await state.json();
        router.push(`/api/auth/onedrive/disconnect?state=${data.state}`);
    };
